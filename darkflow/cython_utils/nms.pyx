...
@cython.wraparound(False)  # turn off negative index wrapping for entire function
@cython.cdivision(True)
cdef NMS9000(float[:, ::1] final_probs , float[:, ::1] final_bbox):
    cdef list boxes = list()
    cdef:
        np.intp_t pred_length,class_length,class_loop,index,index2
    ...
    for index in range(pred_length):
        ...
        for index2 in range(index+1,pred_length):
            ...
            if box_iou_c(final_bbox[index,0],final_bbox[index,1],final_bbox[index,2],final_bbox[index,3],final_bbox[index2,0],final_bbox[index2,1],final_bbox[index2,2],final_bbox[index2,3]) >= 0.25:
                if final_probs[index2,class_loop] > final_probs[index, class_loop] :
                  for sub_loop in range(class_length):
                    final_probs[index, sub_loop] =0
                  break
                for sub_loop in range(class_length):
                  final_probs[index2,sub_loop]=0
        final_probs[index, class_loop] =0 # zero out comparator index
